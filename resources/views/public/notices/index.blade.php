                ...
                @foreach ($notices as $notice)
                    <li class="flex items-start gap-4 p-5 transition hover:bg-slate-50">
                        <div class="hidden w-16 shrink-0 text-center sm:block">
                            <div class="font-display text-lg font-bold text-ink-800">{{ optional($notice->notice_date)->format('j') }}</div>
                            <div class="text-xs uppercase text-slate-400">{{ optional($notice->notice_date)->format('M Y') }}</div>
                        </div>
                        <div class="min-w-0 flex-1">
                            <a href="{{ route('notices.show', $notice) }}" class="font-medium text-ink-900 hover:text-accent-600">
                                {{ $notice->title }}
                                @if ($notice->is_pinned)
                                    <span class="ml-1 text-xs font-semibold uppercase text-accent-600">· Pinned</span>
                                @endif
                            </a>
