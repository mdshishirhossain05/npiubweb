<x-public-layout :title="$post->meta_title ?: $post->title" :description="$post->meta_description ?: $post->excerpt">
    ...
    <article class="mx-auto max-w-3xl px-6 py-16 lg:px-8">
        ...
        @if ($post->excerpt)
            <p class="mt-8 font-display text-xl leading-relaxed text-ink-900">{{ $post->excerpt }}</p>
        @endif
        ...
        <div class="mt-10 border-t border-slate-200 pt-6">
            <a href="{{ route('posts.index') }}" class="text-sm font-semibold text-ink-700 hover:text-accent-600">&larr; Back to all news</a>
        </div>
    </article>
    ...
    @if ($related->isNotEmpty())
        <section class="border-t border-slate-200 bg-slate-50">
            <div class="mx-auto max-w-7xl px-6 py-14 lg:px-8">
                <h2 class="font-display text-xl font-bold text-ink-900">Related news</h2>
                <div class="mt-6 grid gap-8 sm:grid-cols-3">
                    @foreach ($related as $item)
                        <article class="group overflow-hidden rounded-lg border border-slate-200 bg-white">
                            ...
                            <div class="p-5">
                                <h3 class="font-display font-semibold text-ink-900 group-hover:text-accent-600">{{ $item->title }}</h3>
