<x-public-layout title="Events" description="Upcoming and past events at NPI University of Bangladesh.">
    ...
    <div class="mx-auto max-w-5xl px-6 py-16 lg:px-8">
        <section>
            <h2 class="font-display text-2xl font-bold text-navy-900">Upcoming</h2>
            @if ($upcoming->isEmpty())
                ...
                <div class="mt-6 space-y-4">
                    @foreach ($upcoming as $event)
                        <a href="{{ route('events.show', $event) }}"
                           class="flex items-center gap-5 rounded-lg border border-slate-200 bg-white p-5 transition hover:border-brass-500/60">
        ...
        @if ($past->isNotEmpty())
            <section class="mt-12">
                <h2 class="font-display text-2xl font-bold text-navy-900">Past events</h2>
                <ul class="mt-6 divide-y divide-slate-200 rounded-lg border border-slate-200 bg-white">
                    @foreach ($past as $event)
                        <li class="flex items-center justify-between gap-4 p-4">
                            <a href="{{ route('events.show', $event) }}" class="font-medium text-navy-900 hover:text-brass-600">{{ $event->title }}</a>
